...
export default function ChatInterface() {
  ...
  const handleTestGateway = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setResponse('Routing request through gateway...');

    try {
      // Grab the fresh JWT token to pass the middleware Trap
      const { data: { session } } = await supabaseClient.auth.getSession();

      // Ping the Pre-Flight Classifier to verify database permissions
      const res = await fetch('/api/classify', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session?.access_token}` // Crucial for API auth
        },
        body: JSON.stringify({ 
          prompt, 
          requested_tier: tier,
      ...
      // Format the JSON output beautifully in the UI
      setResponse(JSON.stringify(data, null, 2));
    } catch (error: any) {
      setResponse(`Execution Error: ${error.message}`);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-900 text-white font-sans">
      ...
      <header className="flex justify-between items-center p-4 bg-gray-800 border-b border-gray-700">
        <div>
          <h1 className="text-xl font-bold tracking-tight">AI Gateway Terminal</h1>
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4 flex flex-col">
          <form onSubmit={handleTestGateway} className="flex flex-col gap-3">
            ...
            >
              {loading ? 'Processing...' : 'Test Pre-Flight Classifier'}
            </button>
          </form>
