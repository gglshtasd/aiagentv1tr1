  ...
  return (
    <div className="flex h-screen bg-[#0e1117] text-gray-200 font-sans">
      ...
      <div className="w-64 bg-[#161b22] border-r border-gray-800 flex flex-col hidden md:flex">
        <div className="p-4 font-bold tracking-wider text-sm border-b border-gray-800 text-gray-400">
          WORKSPACE
        </div>
        <div className="p-4 flex-1 overflow-y-auto space-y-2">
          {/* History map will go here in Phase 5 */}
          <button className="w-full text-left p-2 rounded bg-gray-800/50 hover:bg-gray-800 text-sm transition-colors text-blue-400">
            + New Chat Session
          </button>
        </div>
      </div>
        ...
        <header className="h-14 border-b border-gray-800 flex items-center justify-between px-6 bg-[#161b22]/50">
          <div className="flex items-center gap-4">
            <select 
              ...
              className="bg-[#0d1117] border border-gray-700 rounded px-3 py-1.5 text-sm font-medium focus:ring-1 focus:ring-blue-500 outline-none"
            >
          ...
          <div className="max-w-3xl mx-auto space-y-8 pb-32">
            {messages.length === 0 ? (
              <div className="text-center text-gray-500 mt-20 flex flex-col items-center">
                ...
                <h2 className="text-xl font-bold text-gray-300">How can I assist you today?</h2>
                <p className="text-sm mt-2">Select a routing mode above to optimize compute.</p>
              </div>
            ) : (
              messages.map((m, i) => (
                <div key={i} className={`flex gap-4 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {m.role === 'assistant' && (
                     <div className="w-8 h-8 rounded-full bg-blue-900/50 border border-blue-800 flex items-center justify-center shrink-0 mt-1">
                        <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                     </div>
                  )}
        ...
        <div className="p-4 bg-gradient-to-t from-[#0d1117] via-[#0d1117] to-transparent absolute bottom-0 left-0 right-0 md:left-64 lg:right-80">
          <div className="max-w-3xl mx-auto relative group">
            <textarea
              ...
              className="w-full bg-[#161b22] border border-gray-700 text-gray-100 rounded-xl pl-4 pr-12 py-4 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none shadow-xl transition-all h-[56px] min-h-[56px] max-h-[200px]"
              rows={1}
            />
            <button
              onClick={sendMessage}
              disabled={loading || !input.trim()}
              className="absolute right-3 top-3 p-1.5 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 text-white rounded-lg transition-colors"
            >
      ...
      <div className="w-80 bg-black border-l border-gray-800 flex flex-col hidden lg:flex">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-[#0d1117]">
           <span className="text-xs font-mono text-gray-400 font-bold tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> TELEMETRY SINK
           </span>
        ...
        <div className="flex-1 p-4 overflow-y-auto font-mono text-[11px] text-gray-400 space-y-1.5 bg-[#0a0a0a]">
          {telemetry.map((log, i) => (
            <div key={i} className={`${log.includes('[ERROR]') || log.includes('[FATAL]') ? 'text-red-400 font-bold' : log.includes('[SYSTEM]') ? 'text-green-400' : 'text-gray-500'}`}>
              {log}
            </div>
          ))}
